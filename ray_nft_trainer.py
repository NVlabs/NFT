...
def compute_dapo_nft_outcome_advantage(token_level_rewards: torch.Tensor,
    ...
    correct_rate = torch.zeros_like(scores)
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            ...
            correct_rate[i] = (id2mean[index[i]] + 1.0) / 2.0
            assert correct_rate[i] < 1.0, f'correct rate is larger than 1.0: {correct_rate[i]}  {id2mean[index[i]]} {id2std[index[i]]}'
            assert correct_rate[i] > -0.0, f'correct rate is less than 0.0: {correct_rate[i]}  {id2mean[index[i]]} {id2std[index[i]]}'
        scores = scores.unsqueeze(-1).tile([1, response_length]) * eos_mask
        correct_rate = correct_rate.unsqueeze(-1).tile([1, response_length])

    return scores, scores, correct_rate
    ...
    def _check_resource_available(self):
        ...
        for _ in range(max_retries):
            try:
                ...
                print(f"Resources not ready, waiting for retry... ({_+1}/{max_retries})")
                import time
                time.sleep(retry_interval)
        
        raise RuntimeError("Exceeded maximum retry attempts, resources still not ready")
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        attention_mask = data.batch['attention_mask']
        response_mask = attention_mask[:, -response_length:]
        advantages, returns, correct_rate = compute_dapo_nft_outcome_advantage(token_level_rewards=token_level_rewards,
                                                                        eos_mask=response_mask,
                                                                        index=index)
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
        data.batch['correct_rate'] = correct_rate
...
class RayNFTTrainer(object):
    ...
    def _create_dataloader(self):
        ...
        if self.config.data.shuffle:
            ...
            train_dataloader_generator.manual_seed(self.config.data.get('seed', 233))
    ...
    def _save_checkpoint(self):
        ...
        prev_ckpt_path = find_latest_ckpt_path(self.config.trainer.default_local_dir)  # None if no lates
        if prev_ckpt_path is not None:
            # delete the previous checkpoint folder
            import shutil
                ...
                with _timer('step', timing_raw):
                    ...
                    if not self.config.algorithm.filter_groups.enable:
                        ...
                        if batch_all is None:
                            ...
                            batch_all = DataProto.concat([batch_all, new_batch])
                        ...
                        if num_prompt_in_batch < prompt_bsz:
                            ...
                            if 'acc' in batch_all.non_tensor_batch.keys():
                                metrics['acc'] = np.mean(batch_all.non_tensor_batch['acc'])
                                metrics['acc_all'] = np.mean(batch_all.non_tensor_batch['acc'])
                                metrics['acc_selected'] = np.mean(batch.non_tensor_batch['acc'])
                            metrics['groupfilter_effective_ratio'] = len(batch) / len(batch_all)
                            metrics['token_level_rewards_sum_acc'] = ((batch.batch['token_level_rewards'].sum(dim=-1) + 1) / 2).float().mean().item()
                            metrics['token_level_rewards_sum_acc_all'] = ((batch_all.batch['token_level_rewards'].sum(dim=-1) + 1) / 2).float().mean().item()
                            metrics['attention_mask_sum'] = batch.batch['attention_mask'].float().sum(dim=-1).mean().item()
                            metrics['attention_mask_sum_all'] = batch_all.batch['attention_mask'].float().sum(dim=-1).mean().item()
                    ...
                    batch.batch['correct_mask'] = (batch.batch['token_level_rewards'].sum(dim=-1) + 1) / 2
                    if self.config.actor_rollout_ref.neg_weight != -1.0:
                        assert batch.batch['correct_mask'].max() <=1.000
                        assert batch.batch['correct_mask'].min() >=0.000
